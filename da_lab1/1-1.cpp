...
int main() {
	...
	TVector<TPair> elems; //Вектор пар «ключ-значение»
	TPair temp;
	...
	TVector<TPair> sortedVector = CountingSort(elems, maxKey); //Сортировка подсчетом

	for (size_t i = 0; i < sortedVector.Size(); ++i) {
		std::cout << sortedVector[i].key << '\t' << sortedVector[i].value << std::endl;
	} //Вывод данных
