...
TSuffixTree::TSuffixTree(string pattern1, string pattern2) : remaining(0), activeLen(0), lengthOfLCS(0) {
    ...
    vector <Vertex *> pathToSub;
    // do the traversal through nodes that belong to both strings to find paths to LCS
    FindLongestSubstrings(root, 0, root, pathToSub);

    list <string> answerStrings;
    int sizeOfAnswer = answers.size();
    for (int i = 0; i < sizeOfAnswer; i++) {
        string answerString;
        int sizeAnswerPathToSubs = answers[i].pathToSubs.size();
        for(int j = 0; j < sizeAnswerPathToSubs; j++) {
            string nextString(answers[i].pathToSubs[j]->begin, answers[i].pathToSubs[j]->end + 1);
            answerString += nextString;
        }
        answerStrings.push_back(answerString);
    }
    answerStrings.sort();

    // Display all the required information
    cout << lengthOfLCS << endl;
    for (list<string>::iterator i = answerStrings.begin(); i != answerStrings.end(); i++) {
        cout << *i << endl;
    }
}
...
// Traversal through nodes that belong to both strings to find path to common substrings which have maximum height from root
void TSuffixTree::FindLongestSubstrings(Vertex *node, int walked, Vertex *begin, vector <Vertex *> pathToSub) {
    if (node->stringNumber.size() == 2) {
        if (node != root) {
            if (walked == 0) {
                pathToSub.clear();
            }
            pathToSub.push_back(node);
            walked += distance(node->begin, node->end) + 1;
            if (walked == lengthOfLCS) { // we reached our substring
                ans.pathToSubs = pathToSub;
                ans.length = walked;
        ...
        for (map<char, Vertex *>::iterator n = node->child.begin(); n != node->child.end(); n++) {
            Vertex * nextChild = n->second;
            FindLongestSubstrings(nextChild, walked, begin, pathToSub);
        }
    }
}
